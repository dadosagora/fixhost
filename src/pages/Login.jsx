  ...
  return (<div className="min-h-screen grid place-items-center bg-slate-50">
    <form onSubmit={onSubmit} className="bg-white p-6 rounded-2xl shadow-sm w-full max-w-sm space-y-4 border">
      ...
      <div className="space-y-1">
        ...
        <input value={username} onChange={e=>setUsername(e.target.value.trim())} className="w-full border rounded-lg px-3 py-2" placeholder="ex.: joao" required />
      </div>
      <div className="space-y-1">
      ...
      <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white rounded-lg py-2 hover:opacity-90 disabled:opacity-50">{loading?'Entrando...':'Entrar'}</button>
    </form></div>)
}
